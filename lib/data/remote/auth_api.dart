...
class AuthApi {
  ...
  validateEmail({required String email}) async {
    if (_fireStoreUsers.doc().snapshots().length == 0)
      ...
      bool _final = true;
      var stream = await _fireStoreUsers.where('email', isEqualTo: email).get();
      print(stream.size);
      if (stream.size == 0) {
        _final = true;
      } else {
  ...
  validatePhone(String phoneNo) async {
    if (_fireStoreUsers.doc().snapshots().length == 0)
      ...
      bool _final = true;
      var stream =
          await _fireStoreUsers.where('phoneNo', isEqualTo: phoneNo).get();
      if (stream.size == 0) {
        _final = true;
      } else {
        _final = false;
  ...
  validateVehicalNo(String vehicalNo) async {
    if (_fireStoreUsers.doc().snapshots().length == 0)
      ...
      bool _final = true;
      var stream =
          await _fireStoreUsers.where('vehicalNo', isEqualTo: vehicalNo).get();
      if (stream.size == 0) {
        _final = true;
      } else {
        _final = false;
  ...
  Future<UserModel> signInWithEmailPassword(
    ...
    exist = await getUsersStream().then((val) {
        ...
        for (var i = 0; i < index; i++) {
          if (phoneNo == val.docs[i].data()['email']) {
            if (password == val.docs[i].data()['password']) {
              ...
              final vehicalImgUrl = val.docs[i].data()['vehicalImgUrl'];
              user.vehicalImgUrl = List<String>.from(vehicalImgUrl);
              user.isDriverVerify = val.docs[i].data()['isDriverVerify'];
              exist = true;
  ...
  Future userUploadImage(String name, File file) async {
    ...
    await _firestore
        ...
        .update({'imgUrl': img}).whenComplete(
            () => LoginViewModel.signedINUser.imgUrl = img);
  ...
  Future updateProfile(String? name, String? mobile, String? password) async {
    ...
    if (name != null || mobile != null || password != null)
      LoginViewModel.showToast("Successfully Update");
  }

  Future createDriverProfile(int age, String cnic, String address) async {
    final user =
        _firestore.collection("users").doc(LoginViewModel.signedINUser.getId);

    await user.update({"age": age}).whenComplete(
        () => LoginViewModel.signedINUser.age = age);
    await user.update({"cnic": cnic}).whenComplete(
        () => LoginViewModel.signedINUser.cnic = cnic);
    await user.update({"address": address}).whenComplete(
        () => LoginViewModel.signedINUser.address = address);
    await user.update({"isDriver": true}).whenComplete(
        () => LoginViewModel.signedINUser.isDriver = true);
  }
  ...
  Future addVehicalImage(String imgName, File imgFile) async {
    ...
    List<String> finalArrray = LoginViewModel.signedINUser.vehicalImgUrl ?? [];
    finalArrray.add(img);

    await user.update({
      "vehicalImgUrl": FieldValue.arrayUnion(finalArrray)
    }).whenComplete(
        () => LoginViewModel.signedINUser.vehicalImgUrl = finalArrray);
  }
  ...
  Future FutureAddVehical(
    ...
    await user.update({"vehicalName": vehicalName}).whenComplete(
        () => LoginViewModel.signedINUser.vehicalName = vehicalName);
    await user.update({"vehicalModel": vehicalModel}).whenComplete(
        () => LoginViewModel.signedINUser.vehicalModel = vehicalModel);
    await user.update({"vehicalNo": vehicalNo}).whenComplete(
        () => LoginViewModel.signedINUser.vehicalNo = vehicalNo);

    await user.update({"isVehicalAdded": true}).whenComplete(
        () => LoginViewModel.signedINUser.isVehicalAdded = true);
  }
}
