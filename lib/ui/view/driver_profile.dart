  ...
  Widget build(BuildContext context) {
    return BaseWidget<LoginViewModel>(
      onModelReady: (p0) {
        ...
        ageController.text =
            LoginViewModel.signedINUser.age.toString().compareTo("-1") == 0
                ? ""
                : LoginViewModel.signedINUser.age.toString();
        cnicController.text =
            LoginViewModel.signedINUser.cnic.toString().compareTo("-1") == 0
                ? ""
                : LoginViewModel.signedINUser.cnic.toString();
        addressController.text = LoginViewModel.signedINUser.address
                    ?.compareTo("Address NotFound") ==
                0
            ? ""
            : LoginViewModel.signedINUser.address ?? "";
      },
      ...
      builder: (context, model, child) {
        ...
        return Scaffold(
          appBar: AppBar(
            ...
            actions: [
              if (LoginViewModel.signedINUser.isDriver!)
                PopupMenuButton<String>(
                  onSelected: (_) {
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => VehicalSetup(),
                        ));
                  },
          ...
          body: ListView(
              ...
              children: [
                if (LoginViewModel.signedINUser.isDriverVerify == false)
                  Center(
                    child: Text(
                ...
                if (model.duplicatePhone)
                  const Text("Phone Number Already exists!",
                      style: TextStyle(color: errorMessage))
                ...
                TextField(
                  ...
                  keyboardType: TextInputType.number,
                  readOnly: cnicController.text.isNotEmpty,
                  ...
                  onChanged: (_) {
                    if (_.length == 5 || _.length == 13) {
                      cnicController.text = '${cnicController.text}-';
                ...
                Container(
                  ...
                  child: model.busy
                      ...
                      : PrimaryButton(
                          ...
                          ontap: () async {
                            model.setupDriverProfile(ageController,
                                cnicController, addressController);
                            model.setBusy(false);
                          },
                        ),
                ),
